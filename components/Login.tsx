...
const Login: React.FC<LoginProps> = ({ onLoginSuccess, isSsoEnabled }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof TypeError && err.message === 'Failed to fetch') {
        setError('Não foi possível conectar ao servidor. Verifique se a API está em execução ou ative o Modo Demo.');
      } else {
  ...
  const handleActivateDemo = async () => {
    setIsLoading(true);
    // 1. Ativa o modo demo localmente
    localStorage.setItem('demo_mode', 'true');
    
    // 2. Simula um atraso curto para feedback visual
    setTimeout(async () => {
        try {
            // 3. O login() agora retornará mockUsers[0] imediatamente via apiService modificado
            const user = await login({ username: 'admin', password: 'any' });
            onLoginSuccess(user);
            // NÃO damos reload aqui para evitar o problema da tela branca no ambiente controlado
        } catch (e) {
