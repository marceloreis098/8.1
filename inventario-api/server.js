...
app.use(session({
    secret: process.env.SESSION_SECRET || 'inventario_secret_key',
    resave: false,
    saveUninitialized: true,
    cookie: { secure: false } // Set to true if using HTTPS behind a proxy correctly configured
}));
...
// Helper function to load SSO config and create strategy
const loadSsoConfig = async () => {
    try {
        const [rows] = await db.promise().query("SELECT * FROM app_config WHERE config_key IN ('ssoUrl', 'ssoEntityId', 'ssoCertificate')");
        const config = {};
        rows.forEach(r => config[r.config_key] = r.config_value);
        
        if (config.ssoUrl && config.ssoEntityId && config.ssoCertificate) {
            return new SamlStrategy(
                {
                    path: '/api/sso/callback',
                    entryPoint: config.ssoUrl,
                    issuer: 'inventario-pro', // Local Entity ID
                    cert: config.ssoCertificate,
                    identifierFormat: null // let IdP decide
                },
                async (profile, done) => {
                    try {
                        // Logic to find or create user based on SAML profile
                        // Usually profile.nameID or profile.email
                        const email = profile.email || profile.nameID; 
                        
                        if (!email) {
                            return done(new Error("No email found in SAML response"), null);
                        }
                        ...
                        if (users.length === 0) {
                            // Option: Auto-provision user or Fail. 
                            // For security, let's Fail unless explicitly allowed, or just map existing users.
                            // Let's try to find by username as fallback if email matches username
                            const [usersByName] = await db.promise().query('SELECT * FROM users WHERE username = ?', [email]);
                             if (usersByName.length > 0) {
                                user = usersByName[0];
                             } else {
                                 return done(new Error("User not found in system"), null);
                             }
                        } else {
                            user = users[0];
                        }
                        ...
                        await db.promise().query('UPDATE users SET ssoProvider = "SAML", lastLogin = NOW() WHERE id = ?', [user.id]);
                        
                        // Clean sensitive data
                        const { password: _, twoFASecret: __, ...cleanUser } = user;
...
// SSO Routes
app.get('/api/sso/login', async (req, res, next) => {
    const strategy = await loadSsoConfig();
    if (!strategy) {
...
app.post('/api/sso/callback', async (req, res, next) => {
    const strategy = await loadSsoConfig();
    if (!strategy) {
        return res.status(500).send("SSO not configured");
    }
    ...
    passport.authenticate('saml', { failureRedirect: '/login', failureFlash: true }, (err, user, info) => {
        if (err) {
        ...
        // Inject user into the HTML to pass to React client
        // A simple way to pass data from backend redirect to frontend SPA
        const userJson = JSON.stringify(user);
        const html = `
            <html>
                <body>
                    <script>
                        localStorage.setItem('currentUser', '${userJson.replace(/'/g, "\\'")}');
